...
interface StoreSelectionScreenProps {
  ...
  onSave: (selectedStoreIds: string[]) => void;
}

export function StoreSelectionScreen({ initialSelectedStores, onSave }: StoreSelectionScreenProps) {
  const insets = useSafeAreaInsets();
  const router = useRouter();
  const { theme } = useUnistyles();
  
  const { stores, isLoading } = useStores({ limit: 50 }); 
  const [selectedStores, setSelectedStores] = useState<string[]>(initialSelectedStores);
